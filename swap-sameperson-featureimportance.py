...
experiment = Experiment(api_key="UuHTEgYku8q9Ww3n13pSEgC8d", project_name="swap-lowerFeatures_impact_both", workspace="enhancing-gradient")
...
def preprocess_image(image_path, apply_mask, num_squares=0, square_size=20, mask_region="lower"):
    ...
    if apply_mask and num_squares > 0:
        if mask_region == "lower":
            # Masking the lower third (approximately the chin area)
            y1 = 74  # Start at roughly two-thirds down the image
        else:
            # Default to central masking if not specified
            y1 = (112 - square_size) // 2
        ...
        for _ in range(num_squares):
            ...
            mask = tf.pad(tensor=tf.ones((square_size, square_size, 3), dtype=tf.float32),
                          paddings=[[y1, 112 - y2], [x1, 112 - x1 - square_size], [0, 0]],
                          mode="CONSTANT", constant_values=0)
            img *= mask
    return preprocess_input(img)

def prepare_dataset(file_path, num_squares, square_size, mask_region):
    def parse_line(line):
    ...
    dataset = dataset.map(lambda x, y, label: (
        (preprocess_image(os.path.join(dataset_dir, x), True, num_squares, square_size, mask_region),
         preprocess_image(os.path.join(dataset_dir, y), True, num_squares, square_size, mask_region)), label),
...
def main():
    ...
    save_directory = "swapping-sameperon-featureimportance"
    ...
    for mask_thickness in mask_thickness_levels:
        ...
        for threshold in thresholds:
            ...
            for pairs_file in pairs_files:
                dataset = prepare_dataset(pairs_file, int(mask_thickness * 10), 20, "lower")
    ...
    plt.title("Accuracy vs. Threshold for Different Mask Thicknesses on Lower Face (Both Images)")
